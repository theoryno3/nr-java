...
public class Rat_interp extends Base_interp {
    ...
    public double rawinterp(final int jl, final double x) throws NRException {
        ...
        hh = abs(x - xa_arr[xa_off + 0]);
        for (i = 0; i < mm; i++) {
            h = abs(x - xa_arr[xa_off + i]);
            if (h == 0.0) {
                dy = 0.0;
                return ya_arr[ya_off + i];
            } else if (h < hh) {
                ns = i;
                hh = h;
            }
            c[i] = ya_arr[ya_off + i];
            d[i] = ya_arr[ya_off + i] + TINY; // The TINY part is needed to
                                              // prevent a rare
            // zero-over-zero
        } // condition.
        y = ya_arr[ya_off + (ns--)];
        for (m = 1; m < mm; m++) {
            for (i = 0; i < mm - m; i++) {
                w = c[i + 1] - d[i];
                h = xa_arr[xa_off + i + m] - x; // h will never be zero, since
                                                // this was tested in the
                                                // initial
                t = (xa_arr[xa_off + i] - x) * d[i] / h; // izing loop.
                dd = t - c[i + 1];
                if (dd == 0.0)
